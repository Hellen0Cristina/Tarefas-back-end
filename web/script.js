...
function exibir_tarefa(tarefa) {
    ...
    if (tarefa.situacao === 'Cancelada'){
        div_tarefa.classList.add('tarefa-cancelada')
    }
    div_tarefa.classList.add('card');
    ...
    div_tarefa.appendChild(prioridade);

    //Nível
    const nivel = document.createElement('span');
    nivel.innerText = 'Nível: ' + tarefa.nivel;
    div_tarefa.appendChild(nivel);

    //Situação
    const situacao = document.createElement('span');
    situacao.innerText = 'Situação: ' + tarefa.situacao;
    div_tarefa.appendChild(situacao);
    ...
    if (tarefa.situacao === 'Cancelada'){
        div_btn.removeChild(btn_cancelar)
    }
    
    btn_cancelar.onclick = async (event) => {
        // chamar API método PUT passando o ID URL
        event.preventDefault()
